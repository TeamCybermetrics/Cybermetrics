...
class TeamWeaknessResponse(BaseModel):
    """Normalized weakness scores per stat (higher = more weakness vs league)."""
...
class ValueScoreRequest(BaseModel):
    """Request body for computing a player's value score.
...
class ValueScoreResponse(BaseModel):
    """Response for player value score calculation."""
    latest_war: float
...
class PlayerValueScore(BaseModel):
    """Minimal view for team value-score listing."""
    id: int
    name: str
    adjustment_score: float
    value_score: float
